...
NetworkEngine::NetworkEngine(const std::string &router_name)
...
bool NetworkEngine::load_topology(const std::string &filename) {
  ...
  if (!file.is_open()) {
    std::cerr << "Error: No se pudo abrir el archivo de topología: " << filename
              << std::endl;
    return false;
  }
  ...
  while (std::getline(file, line)) {
    ...
    if (!(ss >> r_name >> if_name >> loc_port >> rem_ip >> rem_port))
      continue;

    // Solo cargar si la línea corresponde a este router
    if (r_name != router_name_)
      continue;
    ...
    if (bind(link.socket_fd, (struct sockaddr *)&loc_addr, sizeof(loc_addr)) <
        0) {
    ...
    std::cout << "[NetworkEngine] Interfaz " << expanded_name
              << " lista en puerto " << loc_port << " -> " << rem_ip << ":"
              << rem_port << std::endl;
  }
...
void NetworkEngine::start() {
  ...
  for (auto &pair : links_) {
    rx_threads_.emplace_back(&NetworkEngine::rx_loop, this,
                             std::ref(pair.second));
  }
}
...
bool NetworkEngine::send_packet(const std::string &interface_name,
                                const SimulatedPacket &packet) {
  if (links_.find(interface_name) == links_.end())
    return false;

  InterfaceLink &link = links_[interface_name];
  ssize_t sent =
      sendto(link.socket_fd, &packet, sizeof(packet), 0,
